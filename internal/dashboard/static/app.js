...
const filterRunningBtn = document.getElementById('filter-running');
const filterDoneBtn = document.getElementById('filter-done');
const refreshSessionsBtn = document.getElementById('refresh-sessions');
const newerSessionsBtn = document.getElementById('newer-sessions');
const olderSessionsBtn = document.getElementById('older-sessions');
...
const detailCwdEl = document.getElementById('detail-cwd');
const detailCostEl = document.getElementById('detail-cost');
const detailTurnsEl = document.getElementById('detail-turns');
...
function headerNumber(res, name, fallback) {
    const value = Number(res.headers.get(name));
    return Number.isSafeInteger(value) && value >= 0 ? value : fallback;
}
function headerPositiveNumber(res, name, fallback) {
    const value = Number(res.headers.get(name));
    return Number.isSafeInteger(value) && value > 0 ? value : fallback;
}
...
function renderSessionsList() {
    ...
    sessionsListEl.innerHTML = filtered.map(s => {
        ...
        if (s.model.includes('opus'))
            modelBadgeClass = 'text-orange-400 bg-orange-950/20 border-orange-900/30';
        else if (s.model.includes('gpt'))
            modelBadgeClass = 'text-emerald-400 bg-emerald-950/20 border-emerald-900/30';
        else if (s.model.includes('gemini'))
            modelBadgeClass = 'text-cyan-400 bg-cyan-950/20 border-cyan-900/30';
        return `
      <div 
        class="p-4 cursor-pointer flex flex-col space-y-2 transition duration-150 border-l-2 select-none ${isSelected
...
function processLogEvent(event) {
    ...
    `;
        ...
        renderState.currentTurnEl = document.getElementById(`turn-body-${renderState.currentTurnNum}`);
    ...
    if (eventType === 'message_update') {
      ...
      `;
            ...
            renderState.currentThinkingTextEl = thinkContainer.querySelector('#thinking-content');
            window.lucide?.createIcons();
        }
      ...
      `;
            ...
            renderState.currentToolArgsEl = toolEl.querySelector('#tool-args-pre');
            renderState.currentToolArgsRaw = '';
            renderState.toolCallMap.set(toolId, {
                container: toolEl,
                header: toolEl.querySelector('#tool-name-header'),
                argsPre: renderState.currentToolArgsEl,
                resultEl: toolEl.querySelector('#tool-result-container'),
            });
            window.lucide?.createIcons();
        }
...
function createAssistantMessageBlock() {
  ...
  `;
    ...
    renderState.currentAssistantMsgEl = assEl.querySelector('#assistant-message-body');
    window.lucide?.createIcons();
}
// Collapsible helper: Agent Thinking toggle
window.toggleThinkingCollapse = function (el) {
    const content = el.nextElementSibling;
    const chevron = el.querySelector('[data-lucide="chevron-down"], [data-lucide="chevron-up"]');
...
window.toggleResultCollapse = function (el) {
    const container = el.nextElementSibling;
    const chevron = el.querySelector('[data-lucide="chevron-up"], [data-lucide="chevron-down"]');
...
function formatMarkdown(text) {
    ...
    html = sections.map((sec) => {
        if (sec.startsWith('<pre') || sec.startsWith('<ul'))
            return sec;
        return sec.replace(/\n/g, '<br>');
    }).join('');
    return html;
